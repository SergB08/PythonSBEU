...
class Slider:
    def __init__(self, x, y, w, h, initial=1.0):
        self.rect     = pygame.Rect(x, y, w, h)  # прямокутник слайдера
        self.value    = initial                    # поточне значення (0.0 - 1.0)
        self.dragging = False                      # чи перетягується зараз
    ...
    def update(self, events):
        ...
        for e in events:
            ...
            if e.type == pygame.MOUSEBUTTONUP and e.button == 1:
                self.dragging = False
        if self.dragging:
            # оновлює значення на основі позиції миші
...
class OptionRow:
    def __init__(self, label, options, current_index, x, y, btn_w, btn_h,
        ...
        self.label      = label           # назва параметру
        self.options    = options         # список варіантів
        self.selected   = current_index  # поточно вибраний індекс
        self.label_font = pygame.font.SysFont(None, 36, bold=True)
        self.x, self.y  = x, y

        gap = btn_w + 20
        # створює кнопку для кожного варіанту
        self.buttons = [
...
class SettingsMenu:
    WINDOW_OPTIONS = ["Windowed", "Fullscreen", "Borderless"]  # варіанти режиму вікна
    FPS_OPTIONS    = [ 5, 30, 60, 144 ]                        # варіанти обмеження fps
    RES_OPTIONS    = ["1280x720", "1920x1080", "2560x1440"]    # варіанти роздільної здатності
    ...
    def _build(self):
        """Перебудовує позиції всіх елементів для поточного WIDTH/HEIGHT."""
        ...
        # зчитує поточні значення з налаштувань
    ...
    def _apply(self):
        """Зберігає стан віджетів у налаштування та застосовує зміни."""
        # зберігає в модуль налаштувань
        ...
        # перебудовує розміщення для нової роздільної здатності
        self._build()

    def run(self, screen, events):
        """Повертає 'back' коли готово, інакше None."""
        ...
        # повернення та застосування налаштувань
